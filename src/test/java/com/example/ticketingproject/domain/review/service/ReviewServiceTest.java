...
public class ReviewServiceTest {
    ...
    void createReview_success() {
        // given
        // 만약 여기서 에러나면 ReviewRequestDto에 생성자가 있는지 확인!
        ReviewRequestDto requestDto = new ReviewRequestDto("최고의 공연입니다!", 5);
        CustomUserDetails userDetails = new CustomUserDetails(1L, "test@test.com", UserRole.USER);
        ...
        // when
        ReviewResponseDto response = reviewService.createReview(1L, requestDto, userDetails);

        // then
        assertThat(response.getId()).isEqualTo(1L);
        assertThat(response.getContent()).isEqualTo("최고의 공연입니다!");
        assertThat(response.getNickname()).isEqualTo("김규범");
    ...
    void createReview_fail_workNotFound() {
        ...
        // when & then
        assertThatThrownBy(() -> reviewService.createReview(999L, requestDto, userDetails))
    ...
    void findAll_success() {
        ...
        given(reviewRepository.findAllByWorkIdAndDeletedAtIsNull(1L, pageable)).willReturn(reviewPage);
